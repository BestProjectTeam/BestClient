...
{
	constexpr int UNFREEZE_LAG_DELAY_TICKS = 25;
	...
	if(m_FreezeTime > 0)
	{
		...
		m_AliveAccumulation = std::max(m_AliveAccumulation, -UNFREEZE_LAG_DELAY_TICKS);
	}
	else
	{
		m_AliveAccumulation = std::max(m_AliveAccumulation, 1);
		m_AliveAccumulation = std::min(m_AliveAccumulation + 1, UNFREEZE_LAG_DELAY_TICKS);
	}
	...
	{
		m_FreezeAccumulation = std::min(m_FreezeAccumulation + 1, UNFREEZE_LAG_DELAY_TICKS);
	}
	else
	{
