...
bool CBindWheel::OnCursorMove(float x, float y, IInput::ECursorType CursorType)
{
	...
	vec2 SelectorMouse = GameClient()->m_Emoticon.SelectorMouse();
	SelectorMouse += vec2(x, y);
	GameClient()->m_Emoticon.SetSelectorMouse(SelectorMouse);
	return true;
}
...
void CBindWheel::OnRender()
{
	...
	if(!m_Active)
	{
		if(m_WasActive)
		{
			if(g_Config.m_TcResetBindWheelMouse)
				GameClient()->m_Emoticon.SetSelectorMouse(vec2(0.0f, 0.0f));
			if(m_SelectedBind != -1)
	...
	auto &TouchState = GameClient()->m_Emoticon.TouchState();
	const bool WasTouchPressed = TouchState.m_AnyPressed;
	Ui()->UpdateTouchState(TouchState);
	if(TouchState.m_AnyPressed)
	{
		const vec2 TouchPos = (TouchState.m_PrimaryPosition - vec2(0.5f, 0.5f)) * Screen.Size();
		const float TouchCenterDistance = length(TouchPos);
		if(TouchCenterDistance <= s_OuterMouseLimitRadius)
		{
			GameClient()->m_Emoticon.SetSelectorMouse(TouchPos);
		}
		else if(TouchCenterDistance > s_OuterCircleRadius)
		{
			GameClient()->m_Emoticon.SetTouchPressedOutside(true);
		}
	}
	...
	vec2 SelectorMouse = GameClient()->m_Emoticon.SelectorMouse();
	if(length(SelectorMouse) > s_OuterMouseLimitRadius)
		SelectorMouse = normalize(SelectorMouse) * s_OuterMouseLimitRadius;
	GameClient()->m_Emoticon.SetSelectorMouse(SelectorMouse);
	...
	{
		const float SelectedAngle = angle(SelectorMouse);
		if(length(SelectorMouse) > s_InnerOuterMouseBoundaryRadius)
	...
	RenderTools()->RenderCursor(SelectorMouse + vec2(Screen.w, Screen.h) / 2.0f, 24.0f, aAnimationPhase[0]);
}
