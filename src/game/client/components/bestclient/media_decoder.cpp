...
namespace
{
	...
	constexpr int MEDIA_MIN_FRAME_MS = (1000 + MEDIA_FPS_CAP - 1) / MEDIA_FPS_CAP;
	...
	bool DecodeImageWithFfmpegCpu(IGraphics *pGraphics, const unsigned char *pData, size_t DataSize, const char *pContextName, SMediaDecodedFrames &DecodedFrames, const SMediaDecodeLimits &Limits)
	{
		...
		auto ConsumeFrame = [&]() {
			...
			if(DecodeAllFrames)
			{
				...
				if(ImplicitTiming)
				{
					constexpr int MaxReasonableImplicitMs = 200; 
			...
			{
				constexpr int MaxReasonableFallbackUpdateMs = 200; 
			...
			
			{
				...
				{
					constexpr int MaxStartupFrameMsAbs = 200; 
		...
		do
		{
			...
			
			{
				...
				if(Rate.num > 0 && Rate.den > 0)
				{
					...
					constexpr int64_t MaxReasonableFallbackMs = 200; 
