...
namespace
{
	...
	const char *GetChessPieceIcon(char Piece)
	{
		...
		{
		case 'P': return FontIcon::USER;
		case 'N': return FontIcon::ARROWS_LEFT_RIGHT;
		case 'B': return FontIcon::BRUSH;
		case 'R': return FontIcon::SQUARE_PLUS;
		case 'Q': return FontIcon::STAR;
		case 'K': return FontIcon::USER;
...
{
	CUIRect BackButton;
	MainView.HSplitTop(24.0f, &BackButton, &MainView);
	static CButtonContainer s_BackButton;
	if(DoButton_MenuTab(&s_BackButton, BcLocalize("Back to Info"), 0, &BackButton, IGraphics::CORNER_ALL, nullptr, nullptr, nullptr, nullptr, 4.0f))
	{
		CloseBestClientFun();
		return;
	}
	...
	static const SFunGameInfo s_aGames[NUM_FUN_GAMES] = {
		...
		{"Snake", FontIcon::ARROWS_UP_DOWN, "Arrows/WASD, Space restart"},
		{"Minesweeper", FontIcon::TRIANGLE_EXCLAMATION, "LMB open, RMB flag, hover hints"},
		{"Chess", FontIcon::USER, "Font Awesome chess pieces"},
		{"Memory", FontIcon::LAYER_GROUP, "Find all matching pairs"},
		{"Pong", FontIcon::ARROWS_LEFT_RIGHT, "W/S or Up/Down to move paddle"},
	...
	const float BtnH = std::min(BtnW, 95.0f);
...

	{
		...
		for(int i = 0; i < NUM_FUN_GAMES; ++i)
		{
			...
			Ui()->DoLabel(&NameArea, BcLocalize(s_aGames[i].m_pName), FONT_SIZE * 0.78f, TEXTALIGN_MC);
			TextRender()->TextColor(TextRender()->DefaultTextColor());
	...
	if(s_SelectedGame == FUN_GAME_SNAKE)
	{
		...
		const float CellSize = std::min(BoardArea.w / s_Snake.m_BoardW, BoardArea.h / s_Snake.m_BoardH);
		CUIRect Board;
		...
		auto StepSnake = [&]() {
			...
			{
				s_Snake.m_Score++;
				s_Snake.m_BestScore = std::max(s_Snake.m_BestScore, s_Snake.m_Score);
				s_Snake.m_Grow++;
				PlaceFood();
			}
		...
		if(s_Snake.m_Food.x >= 0 && s_Snake.m_Food.y >= 0)
		{
			...
			FoodRect.w = CellSize;
			FoodRect.h = CellSize;
			const float Pad = std::max(1.0f, CellSize * 0.14f);
		...
		for(const ivec2 &Part : s_Snake.m_Body)
		{
			...
			PartRect.w = CellSize;
			PartRect.h = CellSize;
			const float Pad = std::max(1.0f, CellSize * 0.12f);
		...
		if(s_Snake.m_Waiting)
		{
			...
			Ui()->DoLabel(&Overlay, BcLocalize("Press any key to start"), HEADLINE_FONT_SIZE, TEXTALIGN_MC);
		}
	...
	else if(s_SelectedGame == FUN_GAME_MINESWEEPER)
	{
		...
		const float CellSize = std::min(BoardArea.w / s_Mines.m_W, BoardArea.h / s_Mines.m_H);
		CUIRect Board;
		...
		for(int y = 0; y < s_Mines.m_H; ++y)
		{
			for(int x = 0; x < s_Mines.m_W; ++x)
			{
				...
				if(Revealed)
				{
					if(s_Mines.m_vBoard[CurIdx] == -1)
					{
						RenderIconLabel(Cell, FontIcon::TRIANGLE_EXCLAMATION, Cell.h * 0.68f, TEXTALIGN_MC);
					}
	...
	else if(s_SelectedGame == FUN_GAME_CHESS)
	{
		...
		if(s_Chess.m_GameOver)
		{
			if(s_Chess.m_Stalemate)
				pStatus = BcLocalize("Draw - Stalemate");
			else if(s_Chess.m_FiftyMove)
				pStatus = BcLocalize("Draw - Fifty move rule");
			else if(s_Chess.m_Threefold)
				pStatus = BcLocalize("Draw - Threefold repetition");
			else
				pStatus = s_Chess.m_WhiteWon ? BcLocalize("Winner: White") : BcLocalize("Winner: Black");
		}
		else
		{
			pStatus = s_Chess.m_WhiteTurn ? BcLocalize("Turn: White") : BcLocalize("Turn: Black");
		}
		...
		const float BoardSize = std::min(BoardArea.w, BoardArea.h);
		...
		if(s_Chess.m_GameOver)
		{
			...
			const char *pResult;
			if(s_Chess.m_Stalemate)
				pResult = BcLocalize("Draw - Stalemate");
			else if(s_Chess.m_FiftyMove)
				pResult = BcLocalize("Draw - Fifty move rule");
			else if(s_Chess.m_Threefold)
				pResult = BcLocalize("Draw - Threefold repetition");
			else
				pResult = s_Chess.m_WhiteWon ? BcLocalize("White Wins") : BcLocalize("Black Wins");
	...
	else if(s_SelectedGame == FUN_GAME_MEMORY)
	{
		...
		const float GridSize = std::min(BoardArea.w, BoardArea.h);
		...
		for(int y = 0; y < s_Memory.m_Size; ++y)
		{
			for(int x = 0; x < s_Memory.m_Size; ++x)
			{
				...
				const float Pad = std::max(1.0f, CellSize * 0.06f);
	...
	else if(s_SelectedGame == FUN_GAME_PONG)
	{
		...
		const float PreferredW = std::min(ArenaArea.w, ArenaArea.h * 1.8f);
		Arena.w = PreferredW;
		...
		if(!s_Pong.m_Waiting && !s_Pong.m_GameOver)
		{
			...
			if(s_Pong.m_BotY < TargetY)
				s_Pong.m_BotY = std::min(s_Pong.m_BotY + BotSpeed * Dt, 1.0f - PaddleHalf);
			else
				s_Pong.m_BotY = std::max(s_Pong.m_BotY - BotSpeed * Dt, PaddleHalf);
			...
			else if(s_Pong.m_BallPos.x > 1.0f)
			{
				...
				s_Pong.m_BestScore = std::max(s_Pong.m_BestScore, s_Pong.m_PlayerScore);
				ResetPongRound(-1);
			}
	...
	else if(s_SelectedGame == FUN_GAME_BRICK_BREAKER)
	{
		...
		CUIRect Arena = ArenaArea;
		const float PreferredW = std::min(ArenaArea.w, ArenaArea.h * 1.35f);
		Arena.w = PreferredW;
		...
		if(!s_Brick.m_GameOver && !s_Brick.m_Won)
		{
			...
			else
			{
				...
				{
					const float Step = std::min(Remaining, 0.008f);
					Remaining -= Step;
					...
					if(s_Brick.m_BallVel.y > 0.0f && s_Brick.m_BallPos.y + BallRadius >= PaddleY && s_Brick.m_BallPos.y + BallRadius <= PaddleY + 0.03f && fabs(s_Brick.m_BallPos.x - s_Brick.m_PaddleX) <= PaddleHalfW + BallRadius)
					{
						...
						const float Hit = (s_Brick.m_BallPos.x - s_Brick.m_PaddleX) / std::max(0.01f, PaddleHalfW);
					...
					if(s_Brick.m_BallPos.x >= BricksX && s_Brick.m_BallPos.x < 1.0f - BricksX && s_Brick.m_BallPos.y >= BricksY && s_Brick.m_BallPos.y < BricksY + BricksH)
					{
						...
						if(s_Brick.m_vBricks[BIdx])
						{
							...
							s_Brick.m_Score += 10;
							s_Brick.m_BestScore = std::max(s_Brick.m_BestScore, s_Brick.m_Score);

							const float CellCX = BricksX + (BX + 0.5f) * CellW;
							const float CellCY = BricksY + (BY + 0.5f) * CellH;
							const float NX = (s_Brick.m_BallPos.x - CellCX) / std::max(0.001f, CellW);
							const float NY = (s_Brick.m_BallPos.y - CellCY) / std::max(0.001f, CellH);
							if(fabs(NX) > fabs(NY))
		...
		for(int y = 0; y < s_Brick.m_Rows; ++y)
			for(int x = 0; x < s_Brick.m_Cols; ++x)
			{
				...
				const float PadX = std::max(1.0f, Brick.w * 0.06f);
				const float PadY = std::max(1.0f, Brick.h * 0.12f);
				Brick.x += PadX;
				Brick.y += PadY;
				Brick.w -= PadX * 2.0f;
				Brick.h -= PadY * 2.0f;
				const float T = y / (float)std::max(1, s_Brick.m_Rows - 1);
	...
	else if(s_SelectedGame == FUN_GAME_SUDOKU)
	{
		...
		for(int i = 0; i < 3; ++i)
		{
			...
			if(DoButton_Menu(&s_aSudokuDiffButtons[i], BcLocalize(s_apDiffNames[i]), Active, &DiffBtn))
			{
		...
		BoardArea.VSplitRight(std::min(168.0f, BoardArea.w * 0.26f), &PlayArea, &PadArea);
		PadArea.VSplitLeft(MARGIN_SMALL, nullptr, &PadArea);

		const float GridSize = std::min(PlayArea.w, PlayArea.h);
		...
		for(int y = 0; y < 9; ++y)
		{
			for(int x = 0; x < 9; ++x)
			{
				...
				const float Pad = std::max(0.5f, CellSize * 0.04f);
				CUIRect Inner = Cell;
...

		{
			...
			Ui()->DoLabel(&PadTitle, BcLocalize("Numbers"), FONT_SIZE, TEXTALIGN_MC);
			PadArea.HSplitTop(MARGIN_SMALL, nullptr, &PadArea);

			const float Gap = MARGIN_SMALL * 0.6f;
			const float NumBtnSize = std::min((PadArea.w - Gap * 2.0f) / 3.0f, (PadArea.h - Gap * 4.0f) / 4.0f);
			for(int n = 1; n <= 9; ++n)
			{
				const int Row = (n - 1) / 3;
				const int Col = (n - 1) % 3;
			...
			if(DoButton_Menu(&s_aSudokuNumButtons[0], BcLocalize("Clear"), 0, &ClearBtn))
				PlaceNumber(0);
		}
		...
		if(s_Sudoku.m_Won)
		{
			...
			Ui()->DoLabel(&Overlay, BcLocalize("Sudoku Solved"), HEADLINE_FONT_SIZE, TEXTALIGN_MC);
		}
	}
	else if(s_SelectedGame == FUN_GAME_CASINO)
	{
		...
		static const SSymbol s_aSym[6] = {
			...
			{FontIcon::TRIANGLE_EXCLAMATION, ColorRGBA(0.75f, 0.75f, 0.8f, 1.f), "Bomb", 3, true},
		};
		...
		const int ActualReels = std::min(2 + MulIdx + 1, 7);
		...
		if(s_Casino.m_Spinning)
		{
			...
			if(s_Casino.m_SpinTimer >= LastStop)
			{
				...
				else
				{
					...
					s_Casino.m_SpinStreak = std::min(s_Casino.m_SpinStreak + 1, 50);
				}
			}
		}
		...
		if(CanClaim)
		{
			if(DoButton_Menu(&s_ClaimBtn, BcLocalize("Get $200"), 0, &ClaimRect, BUTTONFLAG_LEFT, nullptr, IGraphics::CORNER_ALL, 5.f, 0.f, ColorRGBA(0.2f, 0.65f, 0.2f, 0.9f)))
			{
		...
		Ui()->DoLabel(&MulLabel, BcLocalize("Multiplier:"), FONT_SIZE, TEXTALIGN_ML);
		...
		for(int i = 0; i < ActualReels; ++i)
		{
			...
			else
			{
				...
				if(s_Casino.m_Spinning && s_Casino.m_aLocked[i])
				{
					...
					Ui()->DoLabel(&LockedLabel, BcLocalize("Locked"), FONT_SIZE * 0.75f, TEXTALIGN_MC);
					TextRender()->TextColor(TextRender()->DefaultTextColor());
				}
			}
		...
		{
			const char *pLabel = NotEnoughFunds ? BcLocalize("Not enough funds") : BcLocalize("Spinning...");
		...
		{
			str_format(aBuf, sizeof(aBuf), "%s  (Bet $%d)", BcLocalize("SPIN"), ActualBet);
