...
void CRollbackDemo::SaveRollback()
{
	...
	const int AvailableLength = pReplayRecorder->Length();
	...
	const int WantedLength = std::clamp(g_Config.m_ClReplayLength, 10, 300);
	const int Length = std::min(WantedLength, AvailableLength);
