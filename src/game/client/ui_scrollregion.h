...
public:
	...
	void ScrollHere(EScrollOption Option = SCROLLHERE_KEEP_IN_VIEW, bool Smooth = false); // bestclient: Smooth
	...
	bool Active() const;
	// bestclient
	bool AtBottom(float Epsilon = 2.0f) const { return !ContentOverflows() || m_ScrollPos >= MaxScroll() - Epsilon; }
	// bestclient
